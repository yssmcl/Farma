class Carrie.Views.LearnersReport extends Backbone.Marionette.ItemView
  ...
  loadLos: (ev) ->
    ...
    # update url
    Backbone.history.navigate("my-learners-progress/teams/#{team_id}") if team_id != ""
    @fetchLos(team_id)

  fetchLos: (team_id) ->
    if team_id == ""
      @loSelectTag.html("<option value=\"\">Selectione um OA</option>")
      Backbone.history.navigate("my-learners-progress")
      $(@el).find('.report').html ""
    else
      $.ajax
  ...
  loadReport: (ev) ->
    ...
    if lo_id == ""
      $(@el).find('.report').html ""
      Backbone.history.navigate("my-learners-progress/teams/#{team_id}")
    else
      Backbone.history.navigate("my-learners-progress/teams/#{team_id}/los/#{lo_id}")
      view = new Carrie.CompositeViews.LearnersReportTable(team_id: team_id, lo_id: lo_id)
